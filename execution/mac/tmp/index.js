...
async function frameworkCommand(optionFramework) {
    ...
    const { onPromptState } = core_1.commanderCore;
    if (isSome(optionFramework) && (0, is_1.isTechStack)(optionFramework.value)) {
...
async function libCli(optionLibs) {
    ...
    if (isSome(optionLibs) && (0, is_1.isBoolean)(optionLibs.value) && optionLibs.value) {
        return createOk(createSome([...template_static_1.libsArray]));
    }
    const response = await checkPromiseReturn({
        fn: async () => await (0, prompts_1.default)({
            onState: onPromptState,
...
async function createApp({ appPath, framework, css, libs }) {
    ...
    await (0, install_lib_1.addPackage)({ root, css, libs: selectedPackages });
}
...
async function addPackage({ root, css, libs }) {
    const { isNG, checkPromiseReturn, checkPromiseVoid } = result_1.resultUtility;
    ...
    const addLibs = template_static_1.librarySetting.filter((setting) => libs.some((lib) => lib === setting.lib));
    for (const lib of addLibs) {
        ...
        if (lib.storybook) {
            ...
            const storybookTemplatePath = [
                node_path_1.default.join(__dirname, "lib", "stories", lib.lib + "-" + css),
                node_path_1.default.join(__dirname, "..", "..", "lib", "stories", lib.lib + "-" + css)
            ];
...
async function run() {
    ...
    const { optionName, optionFramework, optionCss, optionUseAllComponents } = core_1.commanderCore;
    const projectName = await (0, name_1.nameCommand)(optionName);
    if (isNG(projectName)) {
        console.error((0, picocolors_1.red)(projectName.err.message));
    ...
    const frameworResult = await (0, framework_1.frameworkCommand)(optionFramework);
    if (isNG(frameworResult)) {
        console.error((0, picocolors_1.red)(frameworResult.err.message));
    ...
    if (isNG(resultSelected)) {
        console.error((0, picocolors_1.red)(resultSelected.err.message));
